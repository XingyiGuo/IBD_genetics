...
REF_DIR="/nobackup/sbcs/chenz27/For_others/GuoLab/Reference/1KG_hg38/EUR/processed"
GWAS_DIR="/nobackup/sbcs/lyul1/IBD/metal_update/withUKB/gc_0.01/GC_Final/clumping/input"
OUT_DIR="/nobackup/sbcs/lyul1/IBD/metal_update/withUKB/gc_0.01/GC_Final/clumping/output_EUR.meta"
EX_DIR="/nobackup/sbcs/lyul1/Reference/1KG_hg38/allsamples/EUR/processed/duplicateid"

mkdir -p "$EX_DIR"
mkdir -p "$OUT_DIR"


GWAS_FILES=("EUR_CD_500k.txt" "EUR_UC_500k.txt" "EUR_IBD_500k.txt")
...
for gwas_file in "${GWAS_FILES[@]}"; do
    ...
    for chr in {1..22}; do
       ...
       #  awk '$2 == "."' "$REF_DIR/chr${chr}.EUR.bim" > "$EX_DIR/missing_id_exclude_chr${chr}.txt"   
        awk '{if(a[$2]++) print $2}' "$REF_DIR/chr${chr}.EUR.bim" > "$EX_DIR/duplicate_exclude_chr${chr}.txt"



        # LD clumping for the current GWAS file and chr   chr1.EUR.bim (excluded "." id) 
                          
        /home/lyul1/plink --bfile "$REF_DIR/chr${chr}.EUR" \
